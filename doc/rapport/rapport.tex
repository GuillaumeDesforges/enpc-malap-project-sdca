...
\paragraph{}
The SDCA method starts from a remark : with standard SGD, the learning rate $\epsilon$ is not adaptative, and it can be more or less complicated to find a good one, which could provide good properties of convergence. That is why the coordinate ascent approach is interesting. Instead of ascenting in the direction of the gradient with a fixed increment, it proposes to update one coordinate to its value that maximizes the objective value. The idea is then to repeat the operation on all coordinates so that it converges to the maximum of the objective function. We can then give it stochastic properties by chosing randomly the coordinate to optimize at each iteration.

\subsection{Some classical loss functions}
...
% TODO décrire ce que fait le SDCA

\subsection{SDCA vs SGD}

% TODO comparer SDCA et SGD dans la théorie (si trop dur, ne pas le faire)
